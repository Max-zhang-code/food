...
status: approved
summary: 订单页增加软删除（取消）功能，以及按提交人筛选订单（全部/我的/某人）
tags: [订单, 删除, 筛选, 云函数]
---
...
| 术语 | 定义 | 防冲突 |
|---|---|---|
| 取消 (cancel) | 将订单状态从 `active`/`completed` 改为 `cancelled`，数据保留不物理删除 | CONTEXT.md 已预留 `cancelled` 终态——本次 **实现该状态**（此前仅定义未编码） |
...
- **做什么**：订单页增加两个能力——(1) 取消订单（软删除，状态变 `cancelled`）；(2) 按提交人筛选订单列表（全部 / 我的 / 某个家人）
- **为谁**：所有家庭成员
- **成功标准**：能取消不需要的订单（误下单或不想做了）；能快速定位自己或某人的订单
- **明确不做**：硬删除（物理删记录）、订单编辑、按审核人角色筛选、取消后恢复、批量取消
...
| 决策 | 选择 | 原因 |
|---|---|---|
| D1：删除方式 | 软删除——新增 `cancelled` 状态 | 保留数据可追溯；CONTEXT.md 原写"不设删除"，本次推翻 |
| D2：取消权限 | 仅订单提交人可取消自己的订单 | 取消是提交人对自己订单的撤销权，与完成订单（任何人可完成）不同 |
| D3：取消范围 | active 和 completed 都可以取消 | 用户选了"软删除推荐"（含已完成订单也可取消） |
| D4：筛选维度 | 按提交人 `user_openid` | 用户确认：全部 / 我的 / 某人的订单 |
| D5：筛选用户列表来源 | 从 orders 集合去重提取 | 只显示实际下过单的人，不用维护独立成员列表 |
| D6：已取消订单展示 | 在历史 tab 中显示，带"已取消"标记 | 与 completed 订单混排，透明度降低 + 标记文字区分 |

### 关键假设

- **假设 1**：家庭成员 3-10 人，订单总量在数百条级别，筛选用户列表去重性能无问题
- **假设 2**：取消是终态——取消后不可恢复、不可重新激活。如需"反取消"由开发者在数据库手动改
- **假设 2b**：仅订单提交人可取消自己的订单——与"完成订单"（任何人可完成）权限模型不同：完成是做饭的人标记做完，取消是提交人撤销自己的下单
- **假设 3**：已取消订单仍在历史列表中可见（而不单独开 tab），家庭场景不需要复杂的订单状态管理

### Top 3 风险

| 风险 | 缓解 |
|---|---|
| R1：取消操作误触 | 前端加确认弹窗（`uni.showModal`），二次确认后才执行 |
...
- 无外部服务依赖
- 无数据迁移需求（`cancelled` 是新增状态值，现有数据无需变更）
...
文字版：`active` → `completed`（完成）/ `cancelled`（取消）；`completed` → `cancelled`（取消）。`cancelled` 是终态，不可再变更。

**Schema 变化**：新增 `cancelled_at: Date` 字段（取消时写入）。
...
```
// 来源：cloudfunctions/cancelOrder/index.js（新建）
...
错误:
  ...
  { code: "NOT_OWNER", message: "只能取消自己的订单" }
  { code: "ALREADY_CANCELLED", message: "该订单已被取消" }
  { code: "CANCEL_FAILED", message: "取消失败，请重试" }
```
...
- 校验 order_id 非空
- 查询订单是否存在
- 校验订单的 `user_openid === OPENID`（仅提交人可取消自己的订单）
- 校验当前状态不是 `cancelled`（幂等保护）
- 更新 status → `cancelled`，写入 `cancelled_at`
...
输入: {
  status: 'active' | 'completed' | 'cancelled' | ('completed' | 'cancelled')[],  // ← 扩展支持数组
  page: 1,
  pageSize: 20,
...
// status 为数组 → 查询匹配任一状态的订单（如历史 tab 传 ['completed', 'cancelled']）
...
// 新增操作：
cancelOrder(order_id: string): Promise<void>   // 调用 cancelOrder 云函数
...
sequenceDiagram
    ...
    rect rgb(255, 240, 240)
        ...
        MP-->>U: 弹窗"确定取消这个订单吗？"
        U->>MP: 确认
        MP->>CF: cancelOrder(order_id)
        CF->>DB: 更新 status=cancelled + cancelled_at
        CF-->>MP: 取消成功
        MP->>MP: 刷新订单列表
    end
```
...
>
> **取消**：用户在订单卡片上看到"取消"按钮 → 点击弹出确认弹窗 → 确认后调用 cancelOrder 云函数 → 成功后 toast "已取消" + 刷新当前列表

#### 筛选栏交互细节

- **「进行中」tab**：筛选栏可见，默认选中"全部"
- **「历史」tab**：筛选栏可见，默认选中"全部"（历史含 completed + cancelled）
...
- **可见性**：仅订单提交人（`user_openid === 当前用户 openid`）可见取消按钮；其他人的订单不显示
- **进行中订单（本人）**：「完成」+「取消」两个按钮
- **进行中订单（他人）**：仅「完成」按钮（与现在一致）
- **历史订单（本人，completed）**：「取消」按钮
- **历史订单（他人，completed）**：无操作按钮
- **历史订单（cancelled）**：无操作按钮，显示"已取消"标记
- **确认弹窗**：`uni.showModal({ title: '取消订单', content: '确定要取消这个订单吗？' })`

#### 流程级约束

- **取消幂等**：已取消的订单再次取消返回 `ALREADY_CANCELLED`，前端拦截（不显示取消按钮）
- **取消失败处理**：云函数返回错误（网络异常 / CANCEL_FAILED / 其他）→ toast 显示具体错误信息，列表不刷新，订单状态保持不变
- **筛选 + 分页**：切换筛选条件时重置 page=1，清空已有列表重新加载
- **取消后刷新**：取消成功后重新请求当前筛选条件下的订单列表
...
| # | 挂载位置 | 动作 | 说明 |
|---|---|---|---|
| 1 | `cloudfunctions/cancelOrder/` | 新增云函数 | 取消订单 |
| 2 | `cloudfunctions/getOrderUsers/` | 新增云函数 | 获取下单用户列表 |
| 3 | `cloudfunctions/getOrders/index.js` | 修改 | 新增可选 `user_openid` 过滤参数 |
| 4 | `src/stores/order.ts` | 修改 | 新增 cancelOrder + getOrderUsers；扩展 fetch 参数 |
| 5 | `src/pages/order/index.vue` | 修改 | 新增筛选栏 UI + 取消按钮 + 确认弹窗 + cancelled 状态展示 |
...
```
1. cancelOrder 云函数：新建 cloudfunctions/cancelOrder/（index.js + package.json）
   退出信号：微信开发者工具云函数测试面板跑通——正常取消 + 重复取消返回 ALREADY_CANCELLED + 不存在的订单返回 ORDER_NOT_FOUND
...
3. order Store 扩展：新增 cancelOrder / getOrderUsers / 扩展现有 fetch 方法签名
   退出信号：TypeScript 类型检查通过（如有）；代码 review 确认接口签名与云函数一致

4. 订单页 UI：筛选栏 + 取消按钮 + 确认弹窗 + cancelled 状态展示
   退出信号：编译通过 + 肉眼验证筛选栏渲染正确 + 取消按钮位置正确

5. 边界态 + harden：空态（无订单/无筛选结果）、加载态、错误态、取消幂等、筛选+分页重置
...
|---|---|---|---|---|
| `src/pages/order/index.vue` | 143 | ~250 | 筛选栏 + 订单卡片列表 + 取消/完成操作 | 偏胖但未到必须拆的程度 |
...
| # | 场景 | 输入/触发 | 期望结果 | 证据类型 |
|---|---|---|---|---|
| S1 | 取消进行中订单 | 进行中订单点「取消」→ 确认弹窗点确定 | 订单从进行中列表消失，历史列表中可见状态为 cancelled | 截图 + 数据库 |
| S2 | 取消已完成订单 | 历史订单（completed）点「取消」→ 确认 | 订单状态变 cancelled，显示"已取消"标记 | 截图 + 数据库 |
| S3 | 筛选「我的」订单 | 筛选栏选「我的」 | 只显示当前用户提交的订单 | 截图 |
| S4 | 筛选某个家人 | 筛选栏选某家人头像/名字 | 只显示该家人的订单 | 截图 |
| S5 | 筛选 + tab 切换 | 在「我的」筛选下切换到历史 tab | 历史 tab 也应用「我的」筛选 | 截图 |
| S6 | 取消后自动刷新 | 取消订单成功后 | 列表自动刷新，不展示已取消订单（进行中 tab） | 截图 |

#### 边界场景

| # | 场景 | 输入/触发 | 期望结果 |
|---|---|---|---|
| S7 | 取消已取消的订单 | 在 cancelled 订单上无取消按钮 | 前端不展示取消按钮，即使用户绕过去调云函数返回 ALREADY_CANCELLED |
| S8 | 筛选无结果 | 选一个没有订单的筛选条件 | 列表为空，显示"暂无订单"空态 |
| S9 | 只有一个人下过单 | 筛选栏用户列表 | 显示"全部"+"我的"+这一个人 |
| S10 | 取消确认取消 | 弹窗点取消 | 不执行取消操作，订单状态不变 |
| S11 | 进行中 tab 筛选后切换回全部 | 筛选「我的」→ 切回「全部」 | 重新显示所有进行中订单 |
| S12 | 网络异常时取消 | cancelOrder 云函数超时 | toast "网络异常请重试"，订单状态不变 |

#### 错误路径

| # | 场景 | 输入/触发 | 期望结果 |
|---|---|---|---|
| S13 | 未登录调 cancelOrder | 未登录直接调云函数 | 返回 NOT_LOGGED_IN |
| S14 | 非本人取消 | A 用户尝试取消 B 用户的订单 | 返回 NOT_OWNER |
| S15 | 缺少 order_id | cancelOrder 不传 order_id | 返回 MISSING_ORDER_ID |
...
| 无取消后恢复 | `cancelOrder` 只写 `status='cancelled'`，无逆向操作 |
| 无批量取消 | 前端无多选/全选，云函数只接受单个 order_id |
...
|---|---|---|---|
| S1 取消进行中 | Step 4 | 截图 + 数据库 | yes |
| S2 取消已完成 | Step 4 | 截图 + 数据库 | yes |
| S3 筛选我的 | Step 4 | 截图 | yes |
| S4 筛选某人 | Step 4 | 截图 | yes |
| S5 筛选+tab切换 | Step 4 | 截图 | yes |
| S6 取消后刷新 | Step 4 | 截图 | yes |
...
**Required Artifacts:** design-review report, checklist completion log, page screenshots (筛选栏各状态 + 取消操作流程 + cancelled 展示), cloud function test evidence.
...
- **CONTEXT.md**：需更新订单术语——状态机描述改为"`active` → `completed` / `cancelled`（cancelled 已实现）"，订单 schema 字段列表新增 `cancelled_at`；删除"不设删除"约束描述（该约束本意是不做物理删除，与本次软删除一致，无需保留此条）
- **无新 ADR**：取消权限沿用家庭信任模型，不引入新结构性决策
